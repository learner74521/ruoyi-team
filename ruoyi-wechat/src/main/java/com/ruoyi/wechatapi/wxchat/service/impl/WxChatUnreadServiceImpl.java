...
public class WxChatUnreadServiceImpl implements IWxChatUnreadService 
{
    ...
    public int updateWxChatUnread(WxChatUnread wxChatUnread)
    {
        ...
        wxChatPeople.setPeopleRoomId(wxChatUnread.getRoomId());
        List<WxChatPeople> wxChatPeopleList=wxChatPeopleMapper.selectWxChatPeopleList(wxChatPeople);
        for (WxChatPeople item:wxChatPeopleList) {
            item.setUpdateTime(DateUtils.getNowDate());
            result=wxChatUnreadMapper.updateWxChatUnreadNum(item);
        }
        return result;
    }
